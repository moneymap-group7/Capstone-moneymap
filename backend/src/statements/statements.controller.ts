...
export class StatementsController {
  constructor(
    ...
    private readonly statementsAdminService: StatementsAdminService,
  ) {}
    ...
    try {
      if (!file) {
        throw new BadRequestException(
          "File is required. Use form-data key: file",
        );
      }
  ...
  @UseGuards(JwtAuthGuard)
  @Get("admin/files")
  async getAllStatementFiles(@Req() req: Request) {
    const userId = getUserIdOrThrow(req);
    return this.statementsAdminService.getAllStatementFiles(userId);
  }

  @UseGuards(JwtAuthGuard)
  @Delete("admin/files/:id")
  async deleteStatementFile(@Param("id") id: string, @Req() req: Request) {
    const userId = getUserIdOrThrow(req);
